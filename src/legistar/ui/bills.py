...
class LegistarBillScraper(LegistarScraper):
    def legislation(self, search_text="", created_after=None, created_before=None):
        ...
        for page in self.search_legislation(search_text, created_after, created_before):
    ...
    def search_legislation(
        self, search_text="", created_after=None, created_before=None
    ):
        """
    ...
    def parse_search_results(self, page):
        ...
        for legislation, headers, row in self.parse_data_table(table):
            ...
            legislation["url"] = (
                self.BASE_URL + legislation_url.split("&Options")[0] + "&FullText=1"
            )
    ...
    def extract_votes(self, action_detail_url):
        ...
        try:
            vote_table = action_detail_page.xpath(
                "//table[@id='ctl00_ContentPlaceHolder1_gridVote_ctl00']"
            )[0]
        except IndexError:
        ...
        for vote, _, _ in votes:
            ...
            vote_list.append(
                (
                    self.VOTE_OPTIONS.get(raw_option, raw_option),
                    vote["Person Name"]["label"],
                )
            )

        action_detail_div = action_detail_page.xpath(
            ".//div[@id='ctl00_ContentPlaceHolder1_pageTop1']"
        )[0]
