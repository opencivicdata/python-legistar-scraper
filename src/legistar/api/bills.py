...
class LegistarAPIBillScraper(LegistarAPIScraper):
    ...
    def topics(self, *args, **kwargs):
        if args:
            ...
            return self.pages(matter_indexes_url, params=kwargs, item_key="IndexId")
    ...
    def history(self, matter_id):
        ...
        for action in actions:
            action["MatterHistoryActionName"] = action[
        ...
        actions = sorted(
            (
                ...
                for action in actions
                if (
                    action["MatterHistoryActionDate"]
        ...
        for action in actions:
            ...
            if current_key != previous_key:
                ...
                self.warning(
                    ...
                    "removed.".format(
                        current_key[0], current_key[1], self.BASE_URL, matter_id
                    )
                )
    ...
    def sponsors(self, matter_id):
        ...
        if spons:
            ...
            return sorted(spons, key=lambda sponsor: sponsor["MatterSponsorSequence"])
    ...
    def _filter_relations(self, relations):
        ...
        sorted_relations = sorted(
            relations,
            key=lambda x: (x["MatterRelationMatterId"], x["MatterRelationFlag"]),
            reverse=True,
        )
    ...
    def legislation_detail_url(self, matter_id):
        ...
        response = self.head(gateway_url, params={k: None for k in self.params})
    ...
    def _missing_votes(self, response):
        ...
        """
        missing = response.status_code == 500 and response.json().get(
            "InnerException", {}
    ...
    def accept_response(self, response, **kwargs):
        ...
        """
        accept = (
            super().accept_response(response)
